...
docker-build-push:
	...
	|| echo "build & push failed. Check if embedded aws gave you the wrong credentials: omni aws ecr get-login-password --region ${AWS_ECR_REGION} --profile ${AWS_ECR_PROFILE} | docker login --username AWS --password-stdin ${DOCKER_REPO}"
